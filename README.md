...
[![Version](https://img.shields.io/badge/version-v1.0.11-blue.svg)](https://github.com/honmannnnn/chatgpt-conversation-tree)
...
- [x] **节点切换响应**: 进一步缩短树节点之间点击切换的等待时间，通过选择器缓存、DOM 快速寻路与紧凑步进延迟实现近似即时响应。
- [x] **SVG 导出边界**: 修复 SVG 图片下载后文字测量与边界溢出问题，增加 `<clipPath>` 与自适应外边距保护。
- [x] **搜索性能与回归测试**: 建立预索引搜索体系，增加针对 1500+ 节点的自动化回归与性能基准测试。
- [x] **大图主线优先布局与直角折线**: 针对 1000+ 节点提供主线垂直向下对齐、分支节点醒目配色折叠与直角折线连接。
- [x] **分支子会话（Fork Chat）识别**: 智能识别独立分叉会话，并在面板顶部提供返回主线入口。
