...
- **💾 本地优先**: 使用 `chrome.storage.local` 与 IndexedDB 保存图结构和消息快照，不上传任何内容。
...
- [ ] **v0.1.0 核心解析器**: 完成 ChatGPT DOM 监听、消息节点解析、双分支数据模型与 IndexedDB 持久化。
- [ ] **v0.2.0 树图面板**: 在页面侧边注入树状图，支持缩放、平移、折叠、节点聚焦和当前路径高亮。
- [ ] **v0.3.0 分支导航**: 支持点击节点定位到原消息，兼容 ChatGPT 的编辑分支与消息版本切换控件。
- [ ] **v0.4.0 搜索与预览**: 增加全文搜索、消息详情侧栏和活跃版本切换。
- [ ] **v0.5.0 导出与快照**: 支持 JSON、Markdown、PNG/SVG 导出，以及对话历史快照。
...
- **状态管理**: Zustand
- **数据存储**: `chrome.storage.local` + IndexedDB
